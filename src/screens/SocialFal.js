 ...
 updateSize = (height) => {
  ...
  renderBody = () => {
    ...
    if(fal){
      ...
      return(
          <View style={{flex:1}}>
            <View style={{backgroundColor:'white',width:'100%',flexDirection:'row',borderColor:'gray',borderBottomWidth:2,height:100}}>
            {
              fal.photos.map(function (foto,index) {

                return (
                   <Image key={index} source={{uri:foto}} style={{flex:1,height:80,margin:10}}></Image>

                  );
              }, this)}
            </View>

            <View style={{flexDirection:'row',justifyContent:'space-between',height:60,borderColor:'gray',borderBottomWidth:2}}>
            ...
            <View style={{alignItems:'center',borderColor:'gray',borderBottomWidth:2,padding:5}}>
              <Text style={{fontWeight:'bold',fontSize:16,color:'teal',paddingBottom:5}}>
                SORUSU
               </Text>
              <Text style={{fontWeight:'normal',fontSize:16}}>
                {fal.question}
            ...
            <View style={{flex:1}}>
              <View style={{backgroundColor:'teal'}}><Text style={{textAlign:'center',color:'white',fontWeight:'bold'}}>Yorumlar</Text></View>
                    ...
                    <View key={index} style={{flexDirection:'row',justifyContent:'space-between',borderBottomWidth:1,borderColor:'gray'}}>
                      ...
                      <View style={{padding:10,flex:2}}>
                        <Text style={{fontWeight:'bold',fontSize:16,marginBottom:5}}>
                          {comment.name} - <Text style={{color:'teal',fontWeight:'normal',fontSize:14}}>
  ...
  render() {
    ...
    return (
      ...
      <View style={styles.container}>
          ...
          {this.renderBody()}
          <KeyboardAvoidingView style={{bottom:this.state.keyboardHeight,flexDirection:'row',padding:3,backgroundColor:'white',position:'absolute',width:'100%'}} >
            <TextInput
              editable={true}
              multiline={true}
              ...
              style={{height:this.state.keyboardHeight>0?80:40,borderColor: 'gray', borderWidth: 1,flex:1,padding:3,backgroundColor:'white'}}
            />
